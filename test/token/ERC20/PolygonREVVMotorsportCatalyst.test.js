...
const implementation = {
  contractName: 'PolygonREVVMotorsportCatalystMock',
  ...
  deploy: async function (initialHolders, initialBalances, deployer) {
    ...
    const catalyst = await artifacts.require('PolygonREVVMotorsportCatalystMock').new(registry.address, deployer, {from: deployer});
    await catalyst.batchMint(initialHolders, initialBalances, {from: deployer});
    return catalyst;
  },
};

describe('PolygonREVVMotorsportCatalyst', function () {
  this.timeout(0);
