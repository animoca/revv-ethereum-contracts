...
const implementation = {
  contractName: 'PolygonREVVMotorsportShardMock',
  ...
  deploy: async function (initialHolders, initialBalances, deployer) {
    ...
    const shard = await artifacts.require('PolygonREVVMotorsportShardMock').new(registry.address, deployer, {from: deployer});
    await shard.batchMint(initialHolders, initialBalances, {from: deployer});
    return shard;
  },
};

describe('PolygonREVVMotorsportShard', function () {
  this.timeout(0);
